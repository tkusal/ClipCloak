...
export const pixDetector: Detector = {
  ...
  detect(text: string, context?: DetectionContext) {
    ...
    while ((match = evpRegex.exec(text)) !== null) {
      ...
      const surrounding =
        context?.surroundingText || text.substring(Math.max(0, match.index - 50), match.index + 50);
    ...
    const contextualPixRegex =
      /(?:pix(?:[\s:-]+chave)?[\s:-]+)([\w.-]+@[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}|\+?55\s?\(?\d{2}\)?\s?9\d{4}-?\d{4})/gi;
